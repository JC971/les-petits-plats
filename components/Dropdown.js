function addTagToSection(tagName, type) {
...
function filterDropdownTags(event) {
	...
	for (let i = 0; i < tags.length; i++) {
		const tag = tags[i];
		tag.style.display = tag.textContent.toLowerCase().includes(filterText)
			? ""
			: "none";
	}
}
...
function getUniqueUstensils(recipes) {
	const ustensils = new Set(
		recipes.flatMap((recipe) => recipe.ustensils).flat()
	);
